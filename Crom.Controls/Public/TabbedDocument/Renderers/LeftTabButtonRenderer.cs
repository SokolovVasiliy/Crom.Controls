...
namespace Crom.Controls.TabbedDocument
{
    ...
    /// </summary>
    public class LeftTabButtonRenderer : TabButtonRenderer
    {
        #region Fields
        ...
        public override void Draw(Rectangle bounds, string text, bool selected, Font font, Graphics graphics)
        {
            using (GraphicsPath button = new GraphicsPath())
            {
                ...
                using (LinearGradientBrush backBrush = new LinearGradientBrush(bounds, GetBackGradient1Color(selected), GetBackGradient2Color(selected), BackGradientMode))
                {
                    ...
                    if (selected)
                    {
                        using (Pen pen = new Pen(GetBackGradient2Color(selected)))
                        {
                            graphics.DrawLine(pen, x1 + 1, y6 + 1, x6 + 1, y1);
                        }
                    }
                }
            }
        ...
        public override void DrawButtonsLine(Graphics graphics, Rectangle panelBounds, Rectangle buttonsPanelBounds)
        {
            ...
            {
                graphics.DrawLine(pen, buttonsPanelBounds.Right, panelBounds.Top, buttonsPanelBounds.Right, panelBounds.Bottom);
            }
        }
        ...
        {
            return new Rectangle(panelBounds.Left, panelBounds.Top - ButtonWidth, ButtonWidth - ButtonLeftMargin, ButtonWidth - ButtonLeftMargin);
        }
        ...
        {
            return new Rectangle(panelBounds.Left, panelBounds.Bottom + 4, ButtonWidth - ButtonLeftMargin, ButtonWidth - ButtonLeftMargin);
        }
        ...
        public override Rectangle GetFirstButtonBounds(Rectangle panelBounds, bool hasScroll, int scrolPos, string text, Font font, Graphics graphics)
        {
            int dY = 4 + PanelMargin;

            if (hasScroll)
            {
                dY += ButtonWidth - ButtonLeftMargin;
            }

            return GetButtonBounds(panelBounds.Left + ButtonLeftMargin + 1, -scrolPos + dY, text, font, graphics);
        }
        ...
        public override Rectangle GetNextButtonBounds(Rectangle previousBounds, string text, Font font, Graphics graphics)
        {
            return GetButtonBounds(previousBounds.Left, previousBounds.Bottom + 2, text, font, graphics);
        }
        ...
        public override Rectangle GetButtonsClipRectangle(Rectangle panelBounds, bool hasScroll, int captionButtonsCount)
        {
            ...
            return new Rectangle(panelBounds.Left + ButtonLeftMargin, panelBounds.Top + dY, ButtonWidth + 1, panelBounds.Bottom - 2 * dY - PanelMargin - dh);
        }
        ...
        {
            int width = ButtonWidth - ButtonLeftMargin;
            int height = ButtonWidth * captionButtonsCount;

            return new Rectangle(panelBounds.Left + ButtonLeftMargin, panelBounds.Bottom - height - 2 * PanelMargin, width, height);
        }
        ...
        public override Rectangle GetClientRectangle(Rectangle panelBounds)
        {
            ...
            clientRectangle.X = panelBounds.Left + ButtonLeftMargin + ButtonWidth + 2;
            clientRectangle.Y = panelBounds.Y;
            clientRectangle.Width = panelBounds.Width - clientRectangle.X;
        ...
        private Rectangle GetButtonBounds(int xPos, int yPos, string text, Font font, Graphics graphics)
        {
