...
namespace Crom.Controls.TabbedDocument
{
    ...
    public abstract partial class ButtonsPanel : UserControl
    {
        ...
        /// <summary>
        /// Occurs when selected index was changed
        /// </summary>
        /// <param name="e">event argument</param>
        ...
        private void UpdatePositions(Graphics graphics, bool forceChangeNotification)
        {
            ...
            if (buttonsCount != 0)
            {
                bounds = ButtonsRenderer.GetFirstButtonBounds(_clientBounds, false, 0, _buttons[0].Text, Font, graphics);
                _buttons[0].SetBounds(bounds);

                for (int index = 1; index < _buttons.Count; index++)
                {
                    bounds = ButtonsRenderer.GetNextButtonBounds(bounds, _buttons[index].Text, Font, graphics);
                ...
                if (_hasScrolls)
                {
                    ...
                    bounds = ButtonsRenderer.GetFirstButtonBounds(_clientBounds, true, scrollPos, _buttons[0].Text, Font, graphics);
                    _buttons[0].SetBounds(bounds);
